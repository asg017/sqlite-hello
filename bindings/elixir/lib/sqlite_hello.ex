defmodule SqliteHello do
  @moduledoc """
  `SqliteHello` module which installs the hello0 and hola0 extensions.
  """
  use Application
  require Logger
  ...
  @doc """
  Returns the configured sqlite_hello version.
  """
  def current_version do
    ...
    case :hex_api_package.get(updated_config, "sqlite_hello") do
      ...
      {:ok, {404, _, _}} ->
        # TODO: remove this fallback once sqlite_hello is published
        "0.1.0-alpha.39"
  ...
  @doc """
  Installs sqlite_hello with `current_version/0`.
  """
  ...
  def bin_path do
    ...
    name = "sqlite-hello-#{current_target}"

    Application.get_env(:sqlite_hello, :path) ||
  ...
  def filename(target) do
    "sqlite-hello-v#{current_version()}-loadable-#{target}.tar.gz"
  end

  @doc """
  The default URL to install SqliteHello from.
  """
  def default_base_url do
    "https://github.com/asg017/sqlite-hello/releases/download/v$version/sqlite-hello-v$version-loadable-$target.tar.gz"
  end

  def loadable_path_hola0() do
    SqliteHello.bin_path() <> "/hola0"
  end

  def loadable_path_hello0() do
    SqliteHello.bin_path() <> "/hello0"
  end
  ...
  def current_target(target_config \\ target_config()) do
    ...
    case os_type do
      {:win32, _} ->
        {:error, "sqlite-hello is not available for architecture: #{arch_str}"}
  ...
  defp fetch_body!(url) do
    ...
    Logger.debug("Downloading sqlite-hello from #{url}")
    ...
    case :httpc.request(:get, {url, []}, http_options(scheme), options) do
      ...
      other ->
        ...
        You may also install the "sqlite_hello" executable manually, \
        see the docs: https://hexdocs.pm/sqlite_hello
        """
    end
  end
  ...
  defp cacertfile() do
    Application.get_env(:sqlite_hello, :cacerts_path) || CAStore.file_path()
  end
